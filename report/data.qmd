...
df_swisspop <- read_excel(paste0(file_path, "swiss_pop.xlsx"), sheet = 1)
```
...
Source - [bfe.admin.ch](https://www.bfs.admin.ch/bfs/fr/home/statistiques/population.assetdetail.26565163.html)
...
```
<<<<<<< HEAD
2.2.7 Political strength per canton

This data set will be used to explore and analyze the effects of political convictions on EVs adoption (per Canton).

The 2 main difficulties with this data set were the following:
  1. Create a process (loop) general enough to clean all the Cantons at the same time, but specific enough for it to work
  2. The Swiss political parties' names have evolved quite a bit over the years.
...
for (i in seq_along(named_data_list)){
  ...
  for (party_name in names(current_dataset)[-1]) {
    ...
    if (length(stance) > 0) {
      # Ensure proper handling of column names
...
for (i in seq_along(named_data_list)) {
  ...
  assign(tibble_name, named_data_list[[i]])
}

  # Create new columns for "Against," "Slightly Against," "Neutral," "Slightly in Favour," and "In Favour"
  #current_dataset <- current_dataset %>%
   # group_by(Date) %>%
    #summarise(
     # Across(c("Against", "Slightly Against", "Neutral", "Slightly In Favour", "In Favour"), 
      #       ~ sum(.x, na.rm = TRUE))
    #)
...
df_swisspop$TotalPopulation <- as.integer(df_swisspop$TotalPopulation)
# Filter out rows where CantonAbbreviation is NA
df_swisspop <- df_swisspop %>%
  filter(!is.na(CantonAbbreviation))

# Display cleaned data
reactable(df_swisspop,
          sortable = TRUE, 
          searchable = TRUE)
>>>>>>> 34e6abdf15e29439886b0d207bed0c4cacc8418c
```

...
```
```{r}
# Creating a new df to keep the original clean
oil_df <- oil_prices_data

# Changing the Date column as Date data-type
oil_df$Date <- as.Date(oil_prices_data$Date, format = "%d-%b-%y") #Specifying it's in the 1900-1999 range
```

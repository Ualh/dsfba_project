...
interactive_plot_seaso_1 <- ggplotly(p_seaso_1)
...
# Convert to an interactive plotly object
interactive_plot_fuel_over_time <- ggplotly(p_fuel_over_time)
...
# Prepare the data with the sum over all years
df_v_map <- df_v %>%
...
# Merge with shapefile data using left_join for efficiency
...
# Create a color palette function for the 'Total' column
color_palette <- colorNumeric(palette = "viridis", domain = map_data_sf$Total)

# Create the leaflet map
leaflet_map <- leaflet(map_data_sf) %>%
  ...
  addPolygons(
    ...
    color = "#FFFFFF",
    fillOpacity = 0.7,
    popup = ~paste(NAME, "<br>Total Registrations: ", Total)
  ) %>%
  addLegend(
    pal = color_palette, 
    values = ~Total, 
...
The above chart represents the oil price evolution through the last two
...
segment of age abovementioned throughout the time. It points out the

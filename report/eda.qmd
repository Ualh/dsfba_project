...
## 3.6 Swiss vs France
### 3.7 Electric vs Hybrid vs Petrol
...
ggplot() +
  ...
  scale_color_manual(values = fuel_colors, labels = c("Diesel", "Electricity", "Conventional hybrid", "Plug-in hybrid", "Petrol"),
                     breaks = c("Diesel", "Electricity", "Conventional hybrid", "Plug-in hybrid", "Petrol")) +
...
```{r}
# Filter electric vehicle data if needed
df_v_electric <- df_v %>% filter(Fuel == "Electricity") |> filter (VehicleType == "Passenger car") # Assuming 'Electric' is the category

# Merge datasets on Date
merged_df <- merge(df_v_electric, df_gtrends, by = "Date")

# Define the transformation function for the secondary axis
transformation_function <- function(x) x * max(merged_df$SearchRatio) / max(merged_df$Count)
# Create a combined bar and line chart
ggplot() +
  geom_bar(data = merged_df, aes(x = Date, y = SearchRatio), stat = "identity", fill = "steelblue") +
  geom_smooth(data = merged_df, aes(x = Date, y = Count), color = "green", method = "loess", span = 0.03) +
  scale_y_continuous(name = "Electric Vehicle Count") +
  labs(title = "Comparison of Electric Vehicle Count and Google Trends Data",
       x = "Date",
       y = "Electric Vehicle Count") +
  theme_minimal()
...
```{r}
head(oil_df)
merged_df <- merge(merged_df, oil_df, by = "Date", all = TRUE)
merged_df

# Create a combined line (for EV count and Oil Price) and bar (for Google Trends) chart with a secondary y-axis
ggplot(merged_df, aes(x = Date)) +
  geom_line(aes(y = Count), color = "red") + # Line for EV count
  geom_line(aes(y = Price), color = "blue") + # Line for Oil Price
  geom_bar(aes(y = SearchRatio), stat = "identity", fill = "steelblue", position = position_dodge()) + # Bar for Google Trends
  scale_y_continuous(name = "Electric Vehicle Count / Oil Price", 
                     sec.axis = sec_axis(~ ., name = "Google Trends Search Ratio", 
                                         breaks = seq(0, 100, by = 20))) + # Secondary axis for Google Trends
  labs(title = "Comparison of Electric Vehicle Count, Oil Prices, and Google Trends Data",
       x = "Date") +
  theme_minimal()
```
